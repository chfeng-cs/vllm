...
def test_on_new_request_disabled_by_default():
    """With early_prefetch=False (default) on_new_request must be a no-op."""
    connector = _make_connector(early_prefetch=False)
    request = create_requests(num_requests=1, num_tokens=10)[0]
